...
class Grid:
    ...
    def play(self):
        ...
        while (minWeight[2] < 9):
            ...
            step += 1
            # print("---Step " + str(step) + "---")
            self.showResult(minWeight[0], minWeight[1])
            # self.showTable()
    ...
    def readInput(self, input):
        ...
        #     row = []
        #     for j in i:
        #         row.append(Block(int(input[i][j]),i,j))
        #     self.grid.append(row)
        #     self.size += 1
    ...
    def showResult(self,i,j):
        
        block = self.grid[i][j]
        # for line in block.line:
        #     remove(line)
        # pygame.display.update()
        if block.bridgeDraw[0] != 0:
            # draw up
            adj = self.findAdjacentNode(i, j, 0)
            if block.bridgeDraw[0] == 1:
                line1 = pygame.draw.line(screen, (0, 0, 0), (block.j*100 +50, block.i*100 + 25), (adj.i*100 + 50 , adj.j - 25), 2)
                block.line.append(line1)
            if block.bridgeDraw[0] == 2:
                line1 = pygame.draw.line(screen, (0, 0, 0), (block.j*100 +40, block.i*100 + 25), (adj.i*100 + 40 , adj.j - 25), 2)
                line2 = pygame.draw.line(screen, (0, 0, 0), (block.j*100 +60, block.i*100 + 25), (adj.i*100 + 60 , adj.j - 25), 2)
                block.line.append(line1)
                block.line.append(line2)
        if block.bridgeDraw[1] != 0:
            # draw right
            pass
            adj = self.findAdjacentNode(i, j, 1)
            if block.bridgeDraw[1] == 1:
                line1 = pygame.draw.line(screen, (0, 0, 0), (block.j*100 + 25, block.i*100 + 50), (adj.i*100 + 25 , adj.j - 25), 2)
                block.line.append(line1)
            if block.bridgeDraw[1] == 2:
                line1 = pygame.draw.line(screen, (0, 0, 0), (block.j*100 + 25, block.i*100 + 40), (adj.i*100 + 25 , adj.j - 25), 2)
                line2 = pygame.draw.line(screen, (0, 0, 0), (block.j*100 + 25, block.i*100 + 60), (adj.i*100 + 25 , adj.j - 25), 2)
                block.line.append(line1)
                block.line.append(line2)
        if block.bridgeDraw[2] != 0:
            # draw down
            adj = self.findAdjacentNode(i, j, 2)
            if block.bridgeDraw[2] == 1:
                line1 = pygame.draw.line(screen, (0, 0, 0), (block.j*100 + 50, block.i*100 + 75), (adj.i*100 + 50 , adj.j - 25), 2)
                block.line.append(line1)
            if block.bridgeDraw[2] == 2:
                line1 = pygame.draw.line(screen, (0, 0, 0), (block.j*100 + 40, block.i*100 + 75), (adj.i*100 + 40 , adj.j - 25), 2)
                line2 = pygame.draw.line(screen, (0, 0, 0), (block.j*100 + 60, block.i*100 + 75), (adj.i*100 + 60 , adj.j - 25), 2)
                block.line.append(line1)
                block.line.append(line2)
        if block.bridgeDraw[3] != 0:
            # draw left
            adj = self.findAdjacentNode(i, j, 3)
            if block.bridgeDraw[3] == 1:
                line1 = pygame.draw.line(screen, (0, 0, 0), (block.j*100 + 75, block.i*100 + 50), (adj.i*100 + 25 , adj.j - 25), 2)
                block.line.append(line1)
            if block.bridgeDraw[3] == 2:
                line1 = pygame.draw.line(screen, (0, 0, 0), (block.j*100 + 75, block.i*100 + 40), (adj.i*100 + 25 , adj.j - 25), 2)
                line2 = pygame.draw.line(screen, (0, 0, 0), (block.j*100 + 75, block.i*100 + 60), (adj.i*100 + 25 , adj.j - 25), 2)
                block.line.append(line1)
                block.line.append(line2)
        pygame.display.update()
        time.sleep(2)
...
pygame.init()
pygame.display.set_caption("Hashi_Search")
...
while not done:
    ...
    for i in range(0,7):
        for j in range(0,7):
            if data[j][i] != 0:
                pygame.draw.circle(screen, (0, 0, 0), (i*100+50, j*100+50), 30, 2)
                screen.blit(myfont.render(str(data[j][i]), True, (255, 0, 0)), (i*100+50-10, j*100+50-15))
                
    d = open("input_hashi.txt", "r")
    g = Grid(d)
    g.play()
    d.close()
    pygame.display.update()
